...
export default function RubberPageError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("🔴 Rubber Page Error:", error);
  }, [error]);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-dvh bg-slate-50 px-6 text-center">
      ...
      <h1 className="text-xl font-black text-slate-900 mb-2">หน้ารับงานมีปัญหา</h1>
      <p className="text-sm text-slate-500 font-bold mb-4">
        กรุณาแคปภาพหน้านี้แล้วส่งให้ทีมพัฒนา
      </p>

      <div className="w-full max-w-sm bg-rose-50 border border-rose-200 rounded-xl p-4 mb-6 text-left">
        <p className="text-[10px] font-black text-rose-400 uppercase mb-1">ERROR:</p>
        <p className="text-xs font-mono text-rose-700 break-all">{error?.message || "Unknown"}</p>
        {error?.digest && (
        ...
        >
          {showDetails ? "ซ่อน" : "แสดง Stack Trace"}
        </button>
