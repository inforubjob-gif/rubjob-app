  ...
  useEffect(() => {
    async function initLiffAndLink() {
      try {
        ...
        if (!accountId || !token) {
          const sessionRes = await fetch(`/api/auth/link-line?type=${type}`);
          if (!sessionRes.ok) throw new Error("กรุณาล็อกอินก่อนเชื่อมต่อ LINE");
        ...
        if (!accountId || !token) {
          setStatus("error");
          setError("ข้อมูลไม่ครบ กรุณากลับไปที่แอปแล้วลองใหม่อีกครั้ง");
          return;
        }
