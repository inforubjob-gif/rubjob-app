  ...
  const handleSave = async () => {
    ...
    try {
      await fetch("/api/users/preferences", {
        ...
        body: JSON.stringify({ userId: rubberId, vehicleType: selectedVehicle })
      });
      router.back();
    } catch (error) {
  ...
  return (
    <div className="flex flex-col min-h-dvh bg-slate-50 pb-24">
      <header className="bg-white px-5 pt-12 pb-4 border-b border-border sticky top-0 z-30">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-bold text-slate-900">{t("rubber.vehicleType") || "Vehicle Type"}</h1>
            <p className="text-xs text-slate-400">{t("rubber.vehicleTypePage.subtitle")}</p>
          </div>
          <Button 
            size="sm" 
            variant="primary" 
      ...
      <div className="p-5 space-y-6 animate-fade-in">
        <p className="text-[10px] font-black text-slate-300 uppercase pl-1">{t("rubber.vehicleTypePage.choosePrimary")}</p>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4">
                      ...
                      <div>
                          <h3 className="font-black text-slate-900 uppercase">{t(`rubber.vehicleTypePage.${key}.name`)}</h3>
                          <p className="text-[10px] text-slate-400 font-bold uppercase">{t(`rubber.vehicleTypePage.${key}.desc`)}</p>
                      </div>
                  </div>
        ...
        <div className="p-5 bg-orange-50 rounded-xl border border-orange-100 mt-4">
            <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                    <p className="text-[10px] text-orange-900 font-black uppercase mb-1">{t("rubber.vehicleTypePage.verificationTitle")}</p>
                    <p className="text-[10px] text-orange-700 font-bold leading-relaxed">
                        {t("rubber.vehicleTypePage.verificationDesc")}
                    </p>
                </div>
            </div>
        </div>
