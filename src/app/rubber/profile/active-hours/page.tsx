...
export default function RubberActiveHoursPage() {
  const router = useRouter();
  const { t } = useTranslation();
  const [isSaving, setIsSaving] = useState(false);
  const [rubberId, setRubberId] = useState<string | null>(null);
  ...
  const handleSave = async () => {
    ...
    try {
      await fetch("/api/users/preferences", {
        ...
        body: JSON.stringify({ userId: rubberId, activeHoursObj: workingHours, activeHours: activeHoursStr })
      });
      router.back();
    } catch (error) {
  ...
  return (
    <div className="flex flex-col min-h-dvh bg-slate-50 pb-24">
      ...
      <header className="bg-white px-5 pt-12 pb-4 border-b border-border sticky top-0 z-30">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-bold text-slate-900">{t("rubber.profile.activeHours")}</h1>
            <p className="text-xs text-slate-400">{t("rubber.activeHoursPage.subtitle")}</p>
          </div>
          <Button 
            size="sm" 
            variant="primary" 
      ...
      <div className="p-5 space-y-6 animate-fade-in">
        {/* Day Selector Tabs */}
        <section>
          <p className="text-[10px] font-black text-slate-300 uppercase pl-1 mb-4">{t("rubber.activeHoursPage.selectDay")}</p>
          <div className="flex overflow-x-auto gap-2 pb-2 no-scrollbar">
             {DAYS.map(day => (
                 ...
                 >
                    {t(`rubber.activeHoursPage.days.${day}`)}
        ...
        <Card className="p-6 border-none shadow-xl shadow-primary/5 rounded-xl bg-white relative overflow-hidden">
           ...
           <div className="flex items-center justify-between mb-8 relative z-10">
              <div>
                 <h3 className="text-2xl font-black text-slate-900 uppercase">{t(`rubber.activeHoursPage.days.${selectedDay}`)}</h3>
                 <p className="text-xs text-slate-400 font-bold uppercase mt-1">{t("rubber.activeHoursPage.configureAvailability")}</p>
              </div>
              <button 
             ...
             <div className="space-y-6 relative z-10">
                 <div className="grid grid-cols-2 gap-4">
                   <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col items-center">
                      <span className="text-[10px] font-black text-slate-400 uppercase mb-2">{t("rubber.activeHoursPage.startTime")}</span>
                      <input 
                         type="time" 
                   ...
                   <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col items-center">
                      <span className="text-[10px] font-black text-slate-400 uppercase mb-2">{t("rubber.activeHoursPage.endTime")}</span>
                      <input 
                         type="time" 
                ...
                <div className="pt-4 flex gap-3">
                   <Button 
                     ...
                     onClick={applyToAll}
                   >
                      <Icons.Clock size={16} /> {t("rubber.activeHoursPage.applyToAll")}
                   </Button>
                </div>
             </div>
           ) : (
             <div className="py-8 text-center bg-slate-50 rounded-xl border-2 border-dashed border-slate-200">
                <p className="text-sm font-black text-slate-400 uppercase">{t("rubber.activeHoursPage.offDuty").replace("{day}", t(`rubber.activeHoursPage.days.${selectedDay}`))}</p>
             </div>
           )}
        </Card>
        ...
        <div className="p-4 bg-orange-50 rounded-xl border border-orange-100 mt-4">
            <div className="flex items-start gap-3">
                ...
                <p className="text-[10px] text-orange-700 font-bold leading-relaxed uppercase">
                    {t("rubber.activeHoursPage.reminder")}
                </p>
            </div>
        </div>
      </div>
