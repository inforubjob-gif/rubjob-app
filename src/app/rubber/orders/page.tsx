...
export default function RubberOrdersPage() {
  ...
  async function fetchOrders(rubberId: string) {
    try {
      ...
      console.error("Failed to fetch rubber orders:", err);
    } finally {
      setIsLoading(false);
    }
  }
        ...
        <div className="flex p-1 bg-slate-200/50 rounded-2xl border border-slate-200/50">
          {(["active", "completed"] as const).map((f) => (
            <button
              ...
              `}
            >
              {f === "active" ? t("rubber.inProgressTab") : t("rubber.completedTab")}
            </button>
          ))}
        </div>
      </div>
      ...
      <main className="flex-1 p-5 space-y-6 pb-28 animate-page-enter stagger">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-sm font-black text-slate-900 uppercase tracking-tight">{t("rubber.noJobs")}</p>
          </div>
        ) : (
          <div className="space-y-4">
             {/* Section Label */}
             <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
               {filter === "active" ? t("rubber.inProgressTab") : t("rubber.completedTab")} ({orders.length})
             </p>
             ...
             {orders.map((order) => {
               ...
               return (
                 <Card 
                   ...
                   onClick={() => router.push(`/rubber/orders/${order.id}`)}
                 >
                     ...
                     <div className="flex-1 min-w-0">
                       <div className="flex items-center justify-between gap-2 mb-1">
                         <h3 className="text-sm font-black text-slate-900 truncate uppercase">
                           {order.storeName || order.customerName || t("rubber.unknownStore")}
                         </h3>
