...
export async function GET() {
  try {
    ...
    const schemaStatements = [
      ...
      `CREATE TABLE IF NOT EXISTS rubber_users (id TEXT PRIMARY KEY, email TEXT UNIQUE NOT NULL, password TEXT NOT NULL, name TEXT, phone TEXT, vehicleType TEXT, status TEXT DEFAULT 'active', address TEXT, idNumber TEXT, licensePlate TEXT, emergencyContact TEXT, createdAt DATETIME DEFAULT CURRENT_TIMESTAMP);`,
      `CREATE TABLE IF NOT EXISTS rubber_documents (id TEXT PRIMARY KEY, rubberId TEXT NOT NULL, type TEXT NOT NULL, status TEXT DEFAULT 'pending', url TEXT, notes TEXT, createdAt DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (rubberId) REFERENCES rubber_users(id) ON DELETE CASCADE);`,
      ...
      // Migration: Add rubber_number to rubber_users
      ...
      `CREATE TABLE IF NOT EXISTS support_tickets (id TEXT PRIMARY KEY, userId TEXT NOT NULL, orderId TEXT, channel TEXT NOT NULL, status TEXT DEFAULT 'open', subject TEXT, assignedAdminId TEXT, createdAt DATETIME DEFAULT CURRENT_TIMESTAMP, updatedAt DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (userId) REFERENCES users(id));`,
