        ...
        <Card className="mb-6 bg-white/10 backdrop-blur-xl border border-white/20 shadow-xl shadow-primary-dark/20 rounded-[2rem] p-4">
           <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                  ...
                  <div>
                      <p className="text-[10px] font-black text-white/50 uppercase tracking-widest leading-none mb-1">{t("staff.profile.workStatus")}</p>
                      <p className="text-sm font-black text-white uppercase tracking-tight">
                        {workStatus ? t("staff.profile.receivingJobs") : t("staff.profile.notReceiving")}
                      </p>
                  </div>
              </div>
              <button 
    ...
    <div className="space-y-4">
      ...
      {orders.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-[2rem] border border-slate-100">
           ...
           <p className="text-xs font-black text-slate-400 uppercase tracking-widest">{t("staff.noJobs") || "No incoming orders"}</p>
        </div>
      ) : (
        orders.map((job) => (
          <Card key={job.id} className="p-4 border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 group">
            ...
            >
              <Icons.Check size={16} className="mr-2" strokeWidth={4} /> {t("staff.receiveFromDriver")}
            </Button>
          </Card>
        ))
      )}
  ...
  return (
    <div className="space-y-4">
      ...
      {orders.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-[2rem] border border-slate-100">
           <div className="text-4xl mb-3">🧼</div>
           <p className="text-xs font-black text-slate-400 uppercase tracking-widest">{t("staff.noJobs") || "No orders being processed"}</p>
        </div>
      ) : (
        orders.map((job) => (
          <Card key={job.id} className={`p-5 border-2 ${job.isExpress ? 'border-red-200 bg-red-50 shadow-red-100' : 'border-slate-100'} shadow-sm rounded-[2.5rem] transition-all hover:shadow-xl`}>
            ...
            <div className="grid grid-cols-2 gap-3 mt-5">
               <Button variant="outline" className="rounded-2xl border-2 font-black text-[10px] py-3.5" onClick={() => router.push(`/staff/orders/${job.id}`)}>
                 {t("common.details")}
               </Button>
               <Button 
