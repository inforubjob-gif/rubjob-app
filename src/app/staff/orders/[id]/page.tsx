...
export default function StaffOrderDetailPage() {
  ...
  // Local state for store input
  const [weight, setWeight] = useState("4.8");
  const [actualItems, setActualItems] = useState("12");
  const [status, setStatus] = useState("delivering_to_store");
  ...
  const handleUpdateStatus = async (newStatus: string) => {
    setIsLoading(true);
    // Real API call: /api/staff/orders/[id]/status
    setTimeout(() => {
        setStatus(newStatus);
        setIsLoading(false);
    }, 800);
  ...
  return (
    <div className="flex flex-col min-h-dvh bg-slate-50">
      ...
      <header className="bg-white px-5 pt-12 pb-4 border-b border-border sticky top-0 z-30">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-bold text-slate-900">{t("staff.manageTask")}</h1>
      ...
      <div className="flex-1 space-y-8 animate-fade-in relative">
        {/* Animated Rider Map Section (Incoming or Ready for Pickup) */}
        {(status === "delivering_to_store" || status === "ready_for_pickup") && (
           <div className="h-44 relative bg-slate-100 overflow-hidden border-b border-slate-200">
              ...
              {/* Rider Icon Bouncing along the path */}
              <div className="absolute top-[60%] left-[65%] -translate-y-1/2 -translate-x-1/2 z-10 flex flex-col items-center">
                 ...
                 <div className="bg-primary text-[8px] font-black text-slate-800 px-2 py-0.5 rounded-full mt-2 shadow-md uppercase tracking-tighter">Wichai Arriving</div>
              </div>

              {/* Store Icon Fixed at one end */}
        ...
        {/* Rider Info Card (Replacing Customer Info for Store Role) */}
        <Card className="p-4 space-y-4 border-2 border-slate-100 shadow-sm rounded-[2rem]">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h3 className="text-sm font-black text-slate-900 uppercase tracking-tight">Somchai Manee</h3>
              <p className="text-[10px] text-primary font-black uppercase tracking-widest leading-none mt-1">Rider #R-2049</p>
            </div>
          ...
          <div className="bg-slate-50 p-4 rounded-3xl flex items-center justify-between border border-slate-100">
             <div className="flex items-center gap-3">
                 ...
                 <div className="text-left">
                   ...
                   <p className="text-xs text-slate-700 font-extrabold">{status === "delivering_to_store" ? t("staff.comingToStore") : t("staff.atStore")}</p>
                 </div>
             </div>
        ...
        {/* Processing Timer (Only if washing) */}
        {status === "washing" && (
           <Card className={`p-6 border-none shadow-2xl ${isExpress ? 'bg-red-600 shadow-red-200' : 'bg-primary shadow-primary/20'} text-white relative overflow-hidden rounded-[2.5rem]`}>
              ...
              <div className="relative z-10 flex items-center justify-between">
                  <div>
                      <p className="text-[11px] font-black text-white/80 uppercase tracking-[0.25em] mb-1.5 leading-none">{t("staff.processing")}</p>
                      <h3 className="text-2xl font-black italic uppercase tracking-tight italic drop-shadow-sm">
                         {isExpress ? t("staff.flashExpress") : t("staff.standardWash")}
                      </h3>
                  </div>
                  <div className="bg-white/20 p-3.5 rounded-2xl backdrop-blur-md border border-white/30 shadow-inner">
                     <CountdownTimer 
                        seconds={isExpress ? 1200 : 3600} 
                        urgentThreshold={900}
                     />
                  </div>
              </div>
        ...
        <div className="grid grid-cols-2 gap-4">
            <Card className="p-4 border-none shadow-sm bg-white border border-slate-100">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">{t("orders.services.laundry")}</p>
                <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-lg bg-orange-50 text-orange-500 flex items-center justify-center shadow-sm">
                        <Icons.WashFold size={18} />
                    </div>
                    <span className="text-xs font-black text-slate-900">Wash & Fold</span>
                </div>
            </Card>
        ...
        <Card className="p-5 space-y-4 shadow-sm border border-slate-100">
            ...
            <div className="space-y-3">
                ...
                <div className="bg-amber-50 p-3 rounded-2xl border border-amber-100 shadow-inner">
                    ...
                    <p className="text-xs text-amber-800 leading-relaxed font-bold">Please separate white clothes. Use low heat for drying.</p>
                </div>
            </div>
        </Card>
        ...
        <div className="space-y-3">
          ...
          {status === "delivering_to_store" && (
            <Button 
                ...
                isLoading={isLoading}
                className="bg-primary text-white hover:bg-primary-dark shadow-xl shadow-primary/20 py-5 text-sm font-black italic rounded-[1.5rem]"
            >
          ...
          {status === "washing" && (
            <Button 
                ...
                isLoading={isLoading}
                className="bg-primary text-white hover:bg-primary-dark shadow-xl shadow-primary/20 py-5 text-sm font-black italic rounded-[1.5rem] uppercase tracking-widest"
            >
          ...
          {status === "ready_for_pickup" && (
            <Button 
                ...
                isLoading={isLoading}
                className="bg-primary text-white hover:bg-primary-dark shadow-xl py-5 text-sm font-black italic rounded-[1.5rem] uppercase tracking-widest"
            >
               <Icons.Check size={20} className="mr-2" strokeWidth={4} /> {t("staff.handoverToDriver") || "ส่งผ้าให้คนขับเรียบร้อย"}
            </Button>
          )}
          ...
          {status === "completed" && (
            <div className="p-10 bg-orange-50 rounded-[2.5rem] border-4 border-dashed border-orange-200 flex flex-col items-center justify-center gap-4 text-center animate-bounce-slow">
                ...
                <div>
                   <h4 className="text-lg font-black text-primary leading-none">{t("staff.handoffSuccess")}</h4>
                   <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2 px-6 italic">{t("staff.handoffDesc")}</p>
                </div>
            </div>
          )}
        </div>
