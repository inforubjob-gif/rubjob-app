...
export default function RiderOrderDetailPage() {
  ...
  const [status, setStatus] = useState("picking_up"); // Initial status for a new assigned job
  ...
  const handleUpdateStatus = async (nextStatus: string) => {
    ...
    setIsLoading(true);
    // Real API call would happen here: /api/rider/orders/[id]/status
    setTimeout(() => {
        setStatus(nextStatus);
        setPhoto(null); // Reset photo for next step
        setIsLoading(false);
    }, 800);
  };
  ...
  const getNextStatus = (current: string) => {
    switch(current) {
        ...
        case "collected_from_user": return "at_store"; // Store takes over
        case "at_store": return "washing"; // Managed by store
        case "washing": return "ready_for_delivery"; // Store completes
      ...
      <div className="flex-1 space-y-6 animate-fade-in relative">
        ...
        <div className="h-48 relative bg-slate-200 overflow-hidden">
           ...
           <div className="absolute bottom-4 right-4 z-20 flex gap-2">
              <button 
                onClick={() => window.open(`https://www.google.com/maps/dir/?api=1&destination=13.7563,100.5018`, "_blank")}
              ...
              <button className="w-10 h-10 rounded-2xl bg-white/90 backdrop-blur-md flex items-center justify-center border border-slate-200 shadow-xl text-primary active:scale-95 transition-all">
                 <Icons.Truck size={20} />
              </button>
           </div>
        </div>
        ...
        <div className="px-5 pt-2 space-y-6">
          ...
          <Card className="p-6 border-none shadow-xl shadow-primary/5 rounded-[2.5rem] bg-white relative overflow-hidden">
             ...
             <div className="flex items-center gap-4 mb-6 relative z-10">
                ...
                <div className="flex-1">
                    <h3 className="text-base font-black text-slate-900 uppercase tracking-tight">Tawan Berkfah</h3>
             ...
             <div className="space-y-4">
                <div className="flex items-start gap-4">
                   ...
                   <div className="flex-1">
                      ...
                      <p className="text-xs font-bold text-slate-700">Rubjob Phrom Phong Store</p>
                   </div>
                </div>
                ...
                <div className="flex items-start gap-4">
                   ...
                   <div className="flex-1">
                      ...
                      <p className="text-xs font-black text-slate-900 leading-relaxed">
                        123/45 Mu Ban Siri, Sukhumvit 101, Bang Chak, Phra Khanong, Bangkok 10260
                      </p>
                   </div>
                </div>
             </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-5 bg-white/80 backdrop-blur-2xl border-t border-slate-100/50 z-40">
           {status === "washing" || status === "at_store" ? (
             ...
             >
                {t(`rider.complete${status.includes('pickup') ? 'Pickup' : status.includes('delivery') ? 'Delivery' : 'Pickup'}`)}
             </Button>
           )}
        </div>
      </div>
