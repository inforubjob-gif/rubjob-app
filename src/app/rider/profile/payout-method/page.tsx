    ...
    <PinLock type="rider" onVerified={() => {}}>
      <div className="flex flex-col min-h-dvh bg-slate-50 text-slate-900">
        <header className="px-5 pt-12 pb-4 border-b border-slate-100 flex items-center gap-3 sticky top-0 bg-white z-10">
          ...
          <h1 className="text-lg font-black text-slate-900">{t("rider.payoutMethodPage.title")}</h1>
        </header>
        ...
        <main className="p-5 space-y-6 animate-fade-in">
          <div className="flex gap-3">
            ...
            >
              ...
              <span className="text-xs font-black uppercase">{t("rider.payoutMethodPage.bankAccount")}</span>
            </button>
            <button
            ...
            >
              ...
              <span className="text-xs font-black uppercase">{t("rider.payoutMethodPage.promptPay")}</span>
            </button>
          </div>

          <Card className="p-6 space-y-5 shadow-xl shadow-slate-200/50">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase ml-1">{t("rider.payoutMethodPage.accountNumber")}</label>
              <input
                type="text"
                value={accountNumber}
            ...
            {method === "bank" && (
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase ml-1">{t("rider.payoutMethodPage.bankName")}</label>
                <select
                  value={bankName}
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase ml-1">{t("rider.payoutMethodPage.accountHolder")}</label>
              <input
                type="text"
                value={accountHolder}
